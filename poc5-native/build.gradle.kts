...
kotlin {
    ...
    fun org.jetbrains.kotlin.gradle.plugin.mpp.KotlinNativeTarget.linuxTarget(libDir: String) {
        compilations.getByName("main").cinterops.create("glfw") {
            ...
            includeDirs(project.file("native/glfw/include"), project.file("native/wayland"))
        }
        // Wayland text-input-v3: the IME path. Under Wayland the app speaks this protocol to the COMPOSITOR
        // (which relays to the input method); it does not talk to IBus directly. Headers from libwayland-dev
        // plus the protocol code generated by wayland-scanner, both staged in native/wayland/.
        compilations.getByName("main").cinterops.create("waylandtext") {
            defFile(project.file("native/waylandtextinput.def"))
            includeDirs(project.file("native/wayland"))
        }
        binaries {
            executable {
                entryPoint = "main"
